...
const Movie = ({movie, savedMovies, props}) => {
  ...
  return (
    <div className={`${visible ? "fade-in" : "fade-out"} h-full w-[350px] inline-block mr-8 cursor-pointer group relative text-white`}>
      <Link
        className="absolute top-0 left-0 border-[1px] border-white p-1 rounded-lg group-hover:block hidden hover:text-white hover:bg-black hover:border-none duration-300"
        to={`/detail/${movie?.id}`}
      >
        Chi tiết
      </Link>
      <img
        className={`${visible ? "fade-in" : "fade-out"} w-full object-cover rounded-2xl group-hover:shadow-lg group-hover:shadow-[#000B58]/70`}
